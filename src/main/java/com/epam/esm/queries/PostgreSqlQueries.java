...
public class PostgreSqlQueries {
    ...
    private static final String date = "TO_CHAR(CURRENT_TIMESTAMP, 'YYYY-MM-DD\"T\"HH24:MI:SS') || '.' || TO_CHAR(CURRENT_TIMESTAMP, 'MS')\n";

    //CERTIFICATES
    public static final String SAVE_GIFT_CERTIFICATE = ("INSERT INTO certificates (certificate_name, description, price, duration, create_date, last_update_date) VALUES (?, ?, ?, ?, %s, %s)").formatted(date,date);
    ...
    public static final String UPDATE_GIFT_CERTIFICATE = ("UPDATE certificates SET certificate_name = COALESCE(?, certificate_name), description = COALESCE(?, description), price = COALESCE(?, price), duration = COALESCE(?, duration), last_update_date = %s WHERE certificate_id = ?").formatted(date);
